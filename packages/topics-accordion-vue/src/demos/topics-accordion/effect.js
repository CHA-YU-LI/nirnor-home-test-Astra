...
export function init(root, options = {}) {
  return instance(root, { duration: 300 }, options, (el, p) => {
    const life = scope(el),
      button = el.querySelector('[data-topic]'),
      panel = el.querySelector('[data-topic-panel]');
    let open = false;
    function render() {
    ...
    life.on(life.motion, 'change', render);
    render();
    life.refresh();
    return {
      ...life,
